...
public class TempRasterization {
    ...
    private final static int[][] ContourX = new int[900][2];

    private static void ScanLine(float x1, float y1, float x2, float y2) {
    ...
    public static void DrawTriangle(List<Point2f> point2fList, PixelWriter pixelWriter) {
        Point2f p0 = point2fList.get(0);
        Point2f p1 = point2fList.get(1);
        Point2f p2 = point2fList.get(2);
        int y;
        ...
        ScanLine(p0.getX(), p0.getY(), p1.getX(), p1.getY());
        ScanLine(p1.getX(), p1.getY(), p2.getX(), p2.getY());
        ScanLine(p2.getX(), p2.getY(), p0.getX(), p0.getY());
        ...
        for (y = 0; y < 900; y++) {
            if (ContourX[y][1] >= ContourX[y][0]) {
                ...
                while (len > 0) {
                    pixelWriter.setColor(x++, y, Color.rgb((int) (Math.random() * 255), (int) (Math.random() * 255), (int) (Math.random() * 255)));
                    len--;
                }
            }
        }
